...
pipeline {
 ...
 stages {
  ...
  stage('Deploy in APIGEE') {
   steps {
	   withMaven(maven: 'apache-maven-3.3.9') {
     bat "mvn -f Z:\Jenkins\workspace\APIGEE-Openshift-JENKINS\apigee-proxy-files\openshift-jenkins-hello\pom.xml install -Pprod -Dusername=${apigee-username} -Dpassword=${apigee-password}"
    }
   }
  }
  /*
